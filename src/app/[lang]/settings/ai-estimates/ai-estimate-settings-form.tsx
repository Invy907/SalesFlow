...
const copy = {
  ja: { title: "AI見積設定", desc: "組織の見積資料と自動入力ルールを管理します。", general: "利用設定", enabled: "AI見積アシストを有効にする", private: "個人資料を本人の推薦に使用する", samples: "単価を自動適用する最小サンプル数", auto: "発行済み見積を自動で確認待ちに追加", retention: "原本の保存日数（空欄は無期限）", web: "公開Webの市場価格調査を許可する", webHelp: "利用者が入力した公開検索語・国・通貨だけをWeb調査AIに送信します。顧客名、社内見積、原本は送信しません。", save: "保存", saved: "保存しました。", admin: "組織管理者のみ変更できます。", provider: "外部データの境界", safe: "生成AIには顧客名とIDを除いた作業説明・承認済み品目・価格・文面のみ送信し、保存しません。PDF・画像原本は送信しません。", library: "AI見積資料を開く" },
  ko: { title: "AI 견적 설정", desc: "조직의 견적 자료와 자동입력 규칙을 관리합니다.", general: "사용 설정", enabled: "AI 견적 도우미 사용", private: "개인 자료를 본인 추천에 사용", samples: "단가 자동적용 최소 표본 수", auto: "발행 견적을 자동으로 검수 대기에 추가", retention: "원본 보관 일수 (비우면 무기한)", web: "공개 웹 시중가 조사 허용", webHelp: "사용자가 입력한 공개 검색어·국가·통화만 웹 조사 AI로 전송합니다. 고객명, 내부 견적, 원본은 전송하지 않습니다.", save: "저장", saved: "저장했습니다.", admin: "조직 관리자만 변경할 수 있습니다.", provider: "외부 데이터 전송 범위", safe: "생성형 AI에는 거래처명과 ID를 제외한 작업 설명·승인 품목·가격·문구만 전송하며 저장하지 않습니다. PDF·이미지 원본은 전송하지 않습니다.", library: "AI 견적 자료함 열기" },
  en: { title: "AI estimate settings", desc: "Manage source and autofill rules for this organization.", general: "Usage settings", enabled: "Enable AI estimate assistant", private: "Use private sources for their owner", samples: "Minimum samples before applying median price", auto: "Add issued estimates to review automatically", retention: "Original retention days (blank means unlimited)", web: "Allow public-web market price research", webHelp: "Only the user's explicit query, country, and currency are sent to the web-research model. Client names, internal estimates, and originals are not sent.", save: "Save", saved: "Saved.", admin: "Only organization admins can change these settings.", provider: "External data boundary", safe: "The generative model receives only the work description and approved items, prices, and wording after client names and IDs are removed. PDF and image originals are not sent.", library: "Open AI estimate library" },
} as const;
