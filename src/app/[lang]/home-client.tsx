...
import type { Dashboard, RecentItem, TaskItem } from "@/lib/db/dashboard";
...
export function HomeClient({ dashboard }: { dashboard: Dashboard }) {
  ...
  const today = now ? formatToday(lang, now) : "";
  const greeting = now ? pickGreeting(ui, now.getHours()) : ui.greeting.afternoon;
  ...
  const { kpi, tasks, recent } = dashboard;
  const delta = deltaPercent(kpi.billedThisMonth, kpi.billedLastMonth);

  const kpiValues: Record<KpiKey, { value: string; sub: string }> = {
    billed: {
      value: yen(kpi.billedThisMonth),
      sub: delta
        ? fillTemplate(ui.kpiSub.deltaVsLastMonth, { delta })
        : ui.kpiSub.noBaseline,
    },
    unpaid: {
      value: yen(kpi.unpaidTotal),
      sub: fillTemplate(ui.kpiSub.count, { count: kpi.unpaidCount }),
    },
    pending: {
      value: String(kpi.draftCount + kpi.awaitingCount),
      sub: fillTemplate(ui.kpiSub.draftAwaiting, {
        draft: kpi.draftCount,
        awaiting: kpi.awaitingCount,
      }),
    },
    duesoon: {
      value: String(kpi.dueSoonCount),
      sub: kpi.overdueCount
        ? fillTemplate(ui.kpiSub.overdueIncluded, { overdue: kpi.overdueCount })
        : ui.kpiSub.noOverdue,
    },
  };

  const groupItems: Record<HomeTaskGroupLabel["key"], TaskItem[]> = {
    duesoon: tasks.dueSoon,
    unsent: tasks.unsent,
    confirm: tasks.awaitingPayment,
  };
  const totalTaskCount =
    tasks.dueSoon.length + tasks.unsent.length + tasks.awaitingPayment.length;
        ...
        <div className="mx-auto w-full max-w-[1260px] px-4 py-6 pb-12 sm:px-6 sm:py-8 sm:pb-14 lg:px-8 lg:py-10 lg:pb-16">
          ...
          <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {(Object.keys(ui.kpiLabels) as KpiKey[]).map((key) => (
              <KpiCard
                key={key}
                href={ui.kpiLabels[key].href}
                label={ui.kpiLabels[key].label}
                tone={ui.kpiLabels[key].tone}
                value={kpiValues[key].value}
                sub={kpiValues[key].sub}
              />
            ))}
          </section>
            ...
            <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
              ...
              {totalTaskCount === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {ui.tasks.groupLabels
                    .filter((group) => groupItems[group.key].length > 0)
                    .map((group) => (
                      <TaskGroup
                        key={group.key}
                        group={group}
            ...
            <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
              ...
              {recent.length === 0 ? (
                ...
                <ul className="divide-y divide-slate-100">
                  {recent.map((item) => (
                    <RecentRow key={`${item.docKind}-${item.id}`} item={item} ui={ui} />
                  ))}
                </ul>
              )}
            </section>
  ...
  return (
    <div className="px-5 py-4">
      ...
      <ul className="space-y-2">
        {items.map((item) => {
          const docLine = [ui.docLabels[item.docKind], item.documentNumber && `#${item.documentNumber}`]
            .filter(Boolean)
            .join(" ");
          const badge =
            group.key === "unsent"
              ? formatDraftAgeLabel(ui.relative, item.dayDiff)
              : formatDueLabel(ui.relative, item.dayDiff);
...
function RecentRow({ item, ui }: { item: RecentItem; ui: HomeContent }) {
  const docLine = [ui.docLabels[item.docKind], item.documentNumber && `#${item.documentNumber}`]
    .filter(Boolean)
    .join(" ");
      ...
      <Link href={item.href} className="flex items-center gap-4 px-5 py-4 transition hover:bg-slate-50">
        ...
        <div className="text-right">
          ...
          <p className="text-[12px] text-slate-500">
            ...
            {formatMinutesAgo(ui.relative, item.minutesAgo)}
          </p>
        </div>
      </Link>
    </li>
