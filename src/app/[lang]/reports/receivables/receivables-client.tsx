...
import type { ReceivablesReport } from "@/lib/db/reports";

const yen = (v: number) => `¥${Math.round(v).toLocaleString("ja-JP")}`;

export function ReceivablesClient({ report }: { report: ReceivablesReport }) {
  ...
  }) => [row.openingBalance, row.billed, row.collected, row.closingBalance];
        ...
        <div className="mt-8 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex items-end gap-3">
            ...
            <button
              type="button"
              onClick={() =>
                router.push(`/reports/receivables?month=${fromMonthField(month)}`)
              }
