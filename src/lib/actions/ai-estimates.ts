...
export async function generateAiEstimateDraft(input: unknown): Promise<ActionResult<{ suggestionId: string; draft: AiEstimateDraft; marketResearch: AiMarketResearchResult | null }>> {
  ...
  if (parsed.data.useWebMarketResearch && !isMarketResearchConfigured()) {
    return { ok: false, error: "웹 조사 API가 설정되지 않았습니다. 서버의 OPENAI_API_KEY를 확인해 주세요." };
  }
  ...
  if (best) {
    const statsQuery = scope.supabase
      .from("ai_estimate_price_stats")
      .select("normalized_name, sample_count, median_price")
      .eq("organization_id", scope.orgId);
    const { data: stats } = parsed.data.clientId
      ? await statsQuery.eq("client_id", parsed.data.clientId)
      : await statsQuery.is("client_id", null);
    const statsMap = new Map((stats ?? []).map((stat) => [stat.normalized_name as string, stat]));
    const minimumSamples = Number(settings?.minimum_price_samples ?? 3);
    const lineValues = best.example.ai_estimate_example_lines as Array<{
      name: string;
      qty: number;
      unit: string | null;
      unit_price: number;
    ...
    const lines = lineValues.map((line) => {
      const stat = statsMap.get(normalizeItemName(line.name));
      const canUseMedian = stat && Number(stat.sample_count) >= minimumSamples;
      if (!canUseMedian) warnings.push(`${line.name}: 가격 표본이 ${minimumSamples}건 미만이라 최근 근거 가격을 사용했습니다.`);
      return {
        name: line.name,
        qty: Number(line.qty),
        unit: line.unit ?? "",
        unitPrice: canUseMedian ? Number(stat.median_price) : Number(line.unit_price),
        taxCategory: line.tax_category,
        confidence: Math.max(0.4, best.score),
        reason: canUseMedian ? `동일 범위 ${stat.sample_count}건의 중앙값` : "가장 유사한 승인 견적의 단가",
      };
    });
    if (marketResearch) {
    ...
    if (marketResearchError) warnings.push(`웹 시중가 조사는 실패했지만 내부 자료 초안은 생성했습니다: ${marketResearchError}`);
    draft = {
      ...
      evidence: ranked.map(({ example, score }) => ({
        ...
        label: [example.issue_date, example.client_name, example.subject].filter(Boolean).join(" · "),
        similarity: score,
      })),
    ...
    draft = {
      ...
      remarks: "",
      evidence: [],
      warnings: ["내부 승인 자료가 없어 웹 시중가 중앙값으로 임시 초안을 만들었습니다. 반드시 출처와 실제 작업 범위를 확인해 주세요."],
    };
  } else {
    return { ok: false, error: marketResearchError ?? "승인된 AI 견적 자료가 없습니다. 자료를 승인하거나 웹 시중가 조사를 선택해 주세요." };
  }
  ...
  const { data: suggestion, error: suggestionError } = await scope.supabase
    ...
    .insert({
      ...
      provider: marketResearch ? "salesflow-retrieval+openai-web-search" : "salesflow-retrieval",
      model: marketResearch ? MARKET_RESEARCH_MODEL : "approved-example-v1",
    })
    .select("id")
    .single();
...
export async function getAiEstimateCapabilities(): Promise<ActionResult<{ webMarketResearchAllowed: boolean; webMarketResearchConfigured: boolean }>> {
  const scope = await getScope();
  if (!scope) return { ok: false, error: "Unauthorized" };
  const { data: settings } = await scope.supabase
    .from("ai_estimate_settings")
