...
export async function getReceipts(orgId: string, filter: DocumentFilter = {}) {
  const supabase = await getSupabaseServerClient();
  const { clientId, status, statusIn, trashed, issueFlag, from, to, query, page = 1, pageSize = 30 } = filter;
  ...
  if (issueFlag !== undefined) q = issueFlag ? q.not("issued_marked_at", "is", null) : q.is("issued_marked_at", null);
...
export async function getReceiptById(id: string) {
  ...
  const { data, error } = await supabase
    .from("receipts")
    .select("*, clients(*), receipt_line_items(*)")
    .eq("id", id)
    .is("deleted_at", null)
    .order("line_no", { referencedTable: "receipt_line_items", ascending: true })
    .maybeSingle();
