...
const copy = {
  ja: { title: "AI見積アシスト", desc: "承認済みの過去見積から品目と価格を提案します。", work: "今回の作業内容", placeholder: "例：A社の採用サイトを5ページ制作。デザインと実装を含む", create: "AI下書きを作成", instant: "すぐ使える価格候補", samples: "件の根拠", result: "提案された下書き", apply: "この内容をフォームに反映", evidence: "参照した見積", empty: "承認済み資料を追加すると候補が表示されます。", library: "AI資料を管理", hide: "閉じる", show: "開く", web: "公開Webで市場価格も調べる", webQuery: "外部に送信する公開検索語", webPlaceholder: "例：日本 Webサイト制作 5ページ 相場 2026", webNotice: "この検索語・国・通貨だけを外部AIに送信します。顧客名や社内見積は入力しないでください。", webDisabled: "管理者の許可またはサーバーAPI設定が必要です。", market: "Web市場価格", sources: "公開情報源", caveats: "確認事項", country: "市場", currency: "通貨" },
  ko: { title: "AI 견적 도우미", desc: "승인된 과거 견적에서 품목과 가격을 제안합니다.", work: "이번 작업 설명", placeholder: "예: A사 채용 사이트 5페이지 제작, 디자인과 구현 포함", create: "AI 초안 만들기", instant: "바로 쓰는 가격 후보", samples: "건 근거", result: "제안된 초안", apply: "이 내용을 폼에 적용", evidence: "참고한 견적", empty: "승인된 자료를 추가하면 추천이 표시됩니다.", library: "AI 자료 관리", hide: "닫기", show: "열기", web: "공개 웹에서 시중가도 조사", webQuery: "외부로 전송할 공개 검색어", webPlaceholder: "예: 일본 웹사이트 제작 5페이지 시중가 2026", webNotice: "이 검색어·국가·통화만 외부 AI로 전송됩니다. 고객명이나 내부 견적 정보는 입력하지 마세요.", webDisabled: "관리자 허용 또는 서버 API 설정이 필요합니다.", market: "웹 시중가", sources: "공개 출처", caveats: "확인 사항", country: "시장", currency: "통화" },
  en: { title: "AI estimate assistant", desc: "Suggest items and prices from approved past estimates.", work: "Describe this job", placeholder: "Example: Five-page recruiting site including design and implementation", create: "Create draft", instant: "Instant price suggestions", samples: "samples", result: "Suggested draft", apply: "Apply to form", evidence: "Evidence", empty: "Add and approve sources to see suggestions.", library: "Manage AI sources", hide: "Close", show: "Open", web: "Also research public-web market prices", webQuery: "Public query sent externally", webPlaceholder: "Example: Japan five-page website design market price 2026", webNotice: "Only this query, country, and currency are sent externally. Do not enter a client name or internal estimate data.", webDisabled: "Admin permission or a server API configuration is required.", market: "Web market prices", sources: "Public sources", caveats: "Caveats", country: "Market", currency: "Currency" },
} as const;
...
export function AiEstimatePanel({
  ...
  const [capabilities, setCapabilities] = useState({ webMarketResearchAllowed: false, webMarketResearchConfigured: false });
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-cyan-50 shadow-sm">
      <div className="flex items-center justify-between gap-4 px-5 py-4 sm:px-6">
        <div className="flex items-center gap-3"><span className="rounded-xl bg-violet-600 p-2 text-white"><WandSparkles className="h-5 w-5" /></span><div><h2 className="font-bold text-slate-900">{ui.title}</h2><p className="text-sm text-slate-600">{ui.desc}</p></div></div>
